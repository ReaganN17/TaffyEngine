...
void Event::schedule() {
	if (check_req && EventHandler::checkRequirements(requirements)) return;

	if (cancel_req) {EventHandler::cancelRequirements(requirements);}
...
void Event::cancelSelf(bool enable) { check_req = enable; }
void Event::cancelOthers(bool enable) { cancel_req = enable; };
